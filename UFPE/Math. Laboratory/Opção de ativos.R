...
last.date <- Sys.Date()
freq.data <- 'weekly'
tickers <- c('SANB11.SA','BBAS3.SA','BBSE3.SA','CMIN3.SA')
tickers
...
# Filtrando os retornos Banco Santander
sant <- l.out$df.tickers %>% 
  na.omit() %>% 
  filter(ticker == 'SANB11.SA') %>% 
  select( ret.closing.prices) %>% 
  rename( Banco_Santander = ret.closing.prices) #Observe que modificamos o nome da coluna.


#Filtrando os retornos da Banco do Brasil
bb <- l.out$df.tickers %>% 
  na.omit() %>% 
  filter(ticker == 'BBAS3.SA') %>% 
  select( ret.closing.prices) %>% 
  rename(Banco_do_Brasil = ret.closing.prices) #Observe que modificamos o nome da coluna.

#Filtrando os retornos do BB seguridade
bbse <- l.out$df.tickers %>% 
  na.omit() %>% 
  filter(ticker == 'BBSE3.SA') %>% 
  select( ret.closing.prices) %>% 
  rename(BB_Seguridade = ret.closing.prices) #Observe que modificamos o nome da coluna.

#Filtrando os retornos da CSN
csn <- l.out$df.tickers %>% 
  na.omit() %>% 
  filter(ticker == 'CMIN3.SA') %>% 
  select( ret.closing.prices) %>% 
  rename(CSN = ret.closing.prices) #Observe que modificamos o nome da coluna.


#Criando os uma variável com todos os retornos
retornos <- cbind(sant, bb, bbse, csn)
